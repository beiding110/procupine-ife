...
Ife.prototype = {
            ...
            if (!_this[key]) {
                Object.defineProperty(_this, key, {
                    ...
                    set: function set(val) {
                        ...
                        _this.catchHandler(val, _this.$data[key], key);

                        _this.emitStorageEvent({
                            type: 'data',
                            key: key,
                            oldValue: _this.$data[key],
                            newValue: val
                        });
                    }
                });
    ...
    emitStorageEvent: function emitStorageEvent(e) {
        sessionStorage.setItem('_ifeEvent', JSON.stringify(e));
    },
    initStorageEventCatcher: function initStorageEventCatcher() {
        if (!!sessionStorage) {
            try {
    ...
    catchHandler: function catchHandler(n, o, key) {
        this.$data[key] = n;
...
module.exports = {
    ...
    scrollin: function scrollin(from, to, cover) {
        Object.keys(from).forEach(function (key, index) {
            if (!cover) {
                to[key] = to[key] ? to[key] : from[key];
            } else {
...
var URLhack = __webpack_require__(/*! ./URLhack */ "./src/plugin/URLhack.js");
...
Router.prototype = {
    ...
    initRouteObserver: function initRouteObserver() {
        var _this = this;

        var changeOuterUrlOnly = false,
            //仅修改外部路由地址，不更新iframe内部视图
        ...
        window.addEventListener('hashchange', function (e) {
            ...
            _this.routeHandler(e);

            if (!_this.$settings.routes) {
                oldPath = routeBreaker(_this.$route.fullPath);
            };
        });

        if (!!this.$settings.routes) return;
        document.querySelector(this.$settings.el).addEventListener('load', function (e) {
            ...
            var newUrl = _this.$win.location.pathname + _this.$win.location.search;
    ...
    routeHandler: function routeHandler(obj, changeUrlOnly) {
        var oldUrl = '',
            newURL = '',
            srcUrl = '',
            toRoute = {},
    ...
    urlBuilder: function urlBuilder(obj) {
        var newPath = '';
    ...
    urlRATestBuilder: function urlRATestBuilder(newPath) {
        var newUrl = '#',
    ...
    urlHasSearch: function urlHasSearch(url) {
        return (/^[^\?=&!]+\?/g.test(url)
        );
    },
    routesPretreatment: function routesPretreatment(routes) {
        ...
        if (Array.isArray(routes)) {
            deepLoop(routes);
        } else if ((typeof routes === 'undefined' ? 'undefined' : _typeof(routes)) === 'object') {
            deepLoop([routes]);
        }
